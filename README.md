...
- `executable` — exactly one of `npm` (package) or `github` (`owner/name`), a templated `urlSuffix` appended to the launcher-owned base URL, and `binPath` for a single-binary agent (omitted for a node bundle). npm verifies via `dist.integrity`; github via the releases-API asset `sha256` digest. Optional `additionalFiles` (array, platform-binary agents only) ships extra helper binaries under their own basename alongside `<binary>-bin` — e.g. Codex's `codex-code-mode-host`.
